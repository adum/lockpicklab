  ...
  (minion.mods ?? []).forEach((modId) => {
    ...
    def.effects?.forEach((effect) => {
      if (effect.type === effectType && "amount" in effect) {
        total += (effect.amount as number) ?? 0;
      }
    });
  });
  return total;
...
function getDeathHealFromMods(minion: CardInstance, cards: CardLibrary): number {
  const mods = Array.isArray(minion.mods) ? minion.mods : [];
  let total = 0;
  mods.forEach((modId) => {
    const def = cards.byId[modId];
...
function returnBorrowedCreatures(state: GameState): void {
...
function applyHealToOpponent(state: GameState, amount: number): void {
...
function applyAnchoredBonuses(state: GameState, cards: CardLibrary): boolean {
  let deathTriggered = false;
  [state.player.board, state.opponent.board].forEach((board) => {
    const desiredBonuses = new Map<number, number>();
...
function applyEndSelfBuffs(state: GameState, cards: CardLibrary): boolean {
  let changed = false;
  [state.player.board, state.opponent.board].forEach((board) => {
    board.forEach((minion) => {
      if (!isCreatureInstance(minion, cards)) {
...
function applyEndMassDeathClones(state: GameState, cards: CardLibrary): void {
...
function applyPlay(state: GameState, action: PlayAction, cards: CardLibrary): GameState {
  ...
  if (def.type === "creature") {
    ...
    const devourEffect = def.effects?.find(
      (effect) => effect.type === "devour_ally"
    );
...
function applyRepeatLastSpell(
  state: GameState,
  effect: { surcharge?: number },
  cards: CardLibrary
...
function applySpellSwapPositions(
  state: GameState,
  target: string | undefined,
  cards: CardLibrary
): void {
...
function applySpellBorrowEnemy(
  state: GameState,
  target: string | undefined,
  effect: { return_multiplier?: number },
  cards: CardLibrary
): void {
...
function applyManaOnMod(state: GameState, cards: CardLibrary): void {
  let total = 0;
  state.player.board.forEach((unit) => {
    const def = cards.byId[unit.card];
    if (!def || def.type !== "effect") {
...
function applyEndManaAdjustments(state: GameState, cards: CardLibrary): void {
  let total = 0;
  state.player.board.forEach((unit) => {
    const def = cards.byId[unit.card];
    if (!def || def.type !== "effect") {
...
function applyEndBossDamage(state: GameState, cards: CardLibrary): void {
  let total = 0;
  state.player.board.forEach((unit) => {
    const def = cards.byId[unit.card];
    if (!def || def.type !== "effect") {
...
function resolveTargetCreature(
  state: GameState,
  target: string,
  cards: CardLibrary
): CardInstance | null {
...
function applyActivateDamage(
  state: GameState,
  source: CardInstance,
  effect: { amount: number; threshold: number },
  target: string,
  cards: CardLibrary
): void {
    ...
    if (def.type === "spell") {
      ...
      if (hasSwap) {
        ...
        const opponentSlots = state.opponent.board
          ...
          .filter((entry) => isCreatureInstance(entry.minion, cards))
          .map((entry) => entry.idx);
        const addPairs = (slots: number[], prefix: "player" | "opponent") => {
