...
import {
  createDfsState,
  isEarlyWin as sharedIsEarlyWin,
  isFinalRoundForSolver as sharedIsFinalRoundForSolver,
  isPastRoundLimit as sharedIsPastRoundLimit,
  stepDfsSearch,
  type DfsSearchState,
  type SolverEngine,
} from "../solver/core.js";
...
export interface GeneratorEngine extends SolverEngine {
...
function normalizeRequiredCards(requiredCards: string[] | undefined): string[] {
...
export function buildPuzzleAttempt(
  ...
  try {
    ...
    if (requiredCards.length > 0) {
      const used = new Set(base.player.hand);
      const missing = requiredCards.filter((cardId) => !used.has(cardId));
      if (missing.length > 0) {
        return {
          hand,
          handLabel,
...
export interface GeneratorSolveState extends DfsSearchState {
  puzzle: Puzzle;
  startState: GameState;
}
...
export function stepSolve(
  ...
  const result = stepDfsSearch(solver, cards, engine, {
    ...
    rejectWin: options?.enforceEarlyWin
      ? (state) =>
          sharedIsEarlyWin(state, options?.targetRounds) ? "early_win" : undefined
      : undefined,
  });
